...
async function webSearch(query) {
  if (!process.env.SERPER_API_KEY) {
    console.warn("[research] SERPER_API_KEY is not set — web search skipped");
    return [];
  }
  ...
  if (!res.ok) {
    console.warn("[research] Serper returned", res.status, "for query:", query);
    return [];
  }
...
export async function researchPerson({ name, email, company }) {
  ...
  if (backgroundSnippets.length) {
    // Combine the best snippets for a fuller picture
    const combined = backgroundSnippets.join(" ").replace(/\s+/g, " ").trim();
    briefParts.push(combined.slice(0, 500));
  }

  const brief = briefParts.join(" ").replace(/\s+/g, " ").trim() || null;
